...
class ProductEditDialog(QDialog):
    ...
    def persist(self):
        ...
        try: rates={key:Decimal(widget.text().replace(",",".") or "0") for key,widget in (("icms_rate",self.icms_rate),("pis_rate",self.pis_rate),("cofins_rate",self.cofins_rate),("ibs_state_rate",self.ibs_state_rate),("ibs_city_rate",self.ibs_city_rate),("cbs_rate",self.cbs_rate))}
        except Exception: QMessageBox.warning(self,"Produto","Alíquotas fiscais inválidas."); return
        data={"barcode":self.barcode.text().strip(),"name":self.name.text().strip(),"price":Decimal(str(self.price.value())),"cost":Decimal(str(self.cost.value())),"stock":Decimal(str(self.stock.value())),"min_stock":Decimal(str(self.min_stock.value())),"unit":self.unit.text().strip().upper() or "UN","category":self.category.text().strip() or "Geral","ncm":self.ncm.text().strip(),"cest":self.cest.text().strip(),"cfop":self.cfop.text().strip() or "5102","origin":self.origin.text().strip(),"icms_cst":self.icms_cst.text().strip(),"pis_cst":self.pis_cst.text().strip(),"cofins_cst":self.cofins_cst.text().strip(),"ibscbs_cst":self.ibscbs_cst.text().strip(),"tax_classification":self.tax_classification.text().strip(),"active":1 if self.active.isChecked() else 0}|rates
