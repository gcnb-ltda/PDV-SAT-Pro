...
class FiscalConfigDialog(QDialog):
    ...
    def nfce_page(self):
        ...
        self.provider=QComboBox(); self.provider.addItems(["Focus NFe","Direto SEFAZ"]); self.provider.setCurrentText(self.data.get("nfce_provider","Focus NFe")); self.focus_token=self.field("focus_token",True); self.tax_regime=QComboBox(); self.tax_regime.addItems(["1 - Simples Nacional","2 - Simples excesso sublimite","3 - Regime normal","4 - MEI"]); self.tax_regime.setCurrentIndex(max(0,int(self.data.get("tax_regime","3"))-1))
    ...
    def persist(self):
        ...
        values={"fiscal_type":self.kind.currentText(),"company_name":self.company_name.text(),"cnpj":self.cnpj.text(),"ie":self.ie.text(),"uf":self.uf.text().upper(),"environment":self.env.currentText(),"operator_name":self.operator_name.text() or "ADMIN","report_role":self.report_role.currentText(),"max_discount_percent":self.max_discount.text(),"sat_dll":self.sat_dll.text(),"sat_code":self.sat_code.text(),"sat_number":self.sat_number.text(),"nfce_provider":self.provider.currentText(),"focus_token":self.focus_token.text(),"tax_regime":str(self.tax_regime.currentIndex()+1),"nfce_certificate":self.cert.text(),"nfce_password":self.cert_pass.text(),"nfce_csc":self.csc.text(),"nfce_csc_id":self.csc_id.text(),"nfce_series":self.series.text(),"nfce_last_number":self.last.text()}
        try:
