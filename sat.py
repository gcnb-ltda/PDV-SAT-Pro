...
class SatDll:
    ...
    def authorize(self, cart, payment, customer_document=""):
        xml = build_cfe_xml(cart, payment, customer_document).encode("utf-8")
...
def build_cfe_xml(cart, payment, customer_document=""):
    details = "".join(
    ...
    if customer_document:
        tag = "CPF" if len(customer_document) == 11 else "CNPJ"
        recipient = f'<dest><{tag}>{customer_document}</{tag}></dest>'
    return f'<CFe><infCFe versaoDadosEnt="0.08">{recipient}<detalhes>{details}</detalhes><pgto><MP><cMP>{escape(payment)}</cMP><vMP>{cart.total}</vMP></MP></pgto></infCFe></CFe>'
